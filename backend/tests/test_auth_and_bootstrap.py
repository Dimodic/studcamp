...
def test_login_and_bootstrap_flow(client):
    ...
    assert any(resource["title"] == "Таблица приезда и отъезда" for resource in bootstrap["resources"])
    assert any(resource["url"] == "https://disk.yandex.ru/d/Qdl9OmGsu697FA" for resource in bootstrap["resources"])
...
def test_mutations_are_persisted(client):
    ...
    response = client.put("/api/v1/projects/preferences", headers=headers, json={"projectIds": project_ids})
    assert response.status_code == 200
