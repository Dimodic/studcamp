...
def mark_story_read(
    story_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> SimpleStatusSchema:
    story = db.scalar(select(Story).where(Story.id == story_id))
    if story is None:
        raise HTTPException(status_code=404, detail="Story not found")

    story_read = db.scalar(
        select(StoryRead).where(
            StoryRead.user_id == current_user.id, StoryRead.story_id == story_id
        )
    )
...
def check_in_event(
    ...
    checkin = db.scalar(
        select(EventCheckIn).where(
            EventCheckIn.user_id == current_user.id, EventCheckIn.event_id == event_id
        )
    )
    if checkin is None:
        checkin = EventCheckIn(
            user_id=current_user.id, event_id=event_id, attendance_status=payload.attendance
        )
        db.add(checkin)
    else:
...
def save_project_preferences(
    ...
    valid_ids = set(db.scalars(select(Project.id).where(Project.id.in_(payload.projectIds))).all())
...
def mark_updates_read(
        ...
        for update in db.scalars(select(OrgUpdate)).all():
            record = db.scalar(
                select(UpdateRead).where(
                    UpdateRead.user_id == current_user.id, UpdateRead.update_id == update.id
                )
            )
            if record is None:
    ...
    for update_id in payload.updateIds or []:
        record = db.scalar(
            select(UpdateRead).where(
                UpdateRead.user_id == current_user.id, UpdateRead.update_id == update_id
            )
        )
        if record is None:
