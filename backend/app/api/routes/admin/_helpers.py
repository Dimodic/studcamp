...
def get_or_404(db: Session, model: type[ModelType], entity_id: str, *, field_name: str = "id") -> ModelType:
...
def parse_enum(enum_cls: type[EnumType], raw_value: str, field_name: str) -> EnumType:
    try:
...
def ensure_unique_email(db: Session, email: str | None, *, exclude_user_id: str | None = None) -> None:
    if not email:
        return
...
def get_manageable_event(db: Session, current_user: User, event_id: str) -> Event:
    ...
    if current_user.role == UserRole.organizer:
        return event
    if current_user.role == UserRole.teacher and _event_owned_by_teacher(db, current_user.id, event_id):
        return event
    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
...
def ensure_manageable_related_event_id(db: Session, current_user: User, event_id: str | None) -> None:
    if event_id is None:
